...
论文从变分结构出发。下面的 Lagrangian 只用于说明变量扩展的机制，是本笔记的示意性重构，不应当作该文对多维 SGN 的逐项定义：
...
不同论文记号可能把松弛参数写作 $\alpha$、$\varepsilon$ 或 $c$；其作用相同：当 $\epsilon\to0$ 时强制 $q\to D_th$，恢复目标色散结构；有限 $\epsilon$ 时得到一阶扩展系统。具体辅助变量、缩放和能量项须以原文方程为准。

...
### 3. 主要结论是什么？
- **三条件充分性定理**：满足 (A)(B)(C) 的 G₂ 近似 → G(U) 与 L 自动满足全部守恒律（数/动量/角动量/能量）及 f-sum rule 等求和规则；数守恒还可推出规范不变性（与纵向电导求和规则等价）。
...
这类似 Hartree-Fock，但把交换相互作用 $V$ 替换为动态屏蔽相互作用 $V_s$。对 $\Sigma_s$ 取变分时，除 Hartree 与屏蔽交换顶点外，还要变分 $V_s[G]$。由

\[
\frac{\delta V_s}{\delta G}
\sim
V_s\,G\,V_s
+
V_s\,G\,V_s
...
GG\,V_s\,L
+
GGGG\,(V_sV_s+V_sV_s)\,L.
\]

最后一项给出类似 Boltzmann 碰撞项的结构，碰撞截面正比于 $|V_s|^2$。这正适合高密度电子气、等离子体以及电子-离子双组分金属模型，因为 $V_s$ 的极点包含等离激元，也可在双组分情形中包含声波型集体模。

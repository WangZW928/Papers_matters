...
1. 物理网格 GNN（MeshGraphNet 类）在 AMR 层次网格上的表达力：消息传递 GNN 不超过 1-WL 的结论对“带几何特征的物理网格”是否仍构成实质限制？当节点特征包含坐标/面积/法向等连续几何量时，WL 等价类划分如何被打破，过平滑问题在非规则网格（尺度差异巨大的粗细层）上是否更严重？
2. 谱域 vs 空间域 GNN 对网格学习的选择：Laplacian 谱滤波的低通性质与 NTK 谱偏置叠加，会如何影响物理 surrogate 对小尺度/高频流动结构的拟合？在自适应网格上重新建图时，谱方法依赖的固定邻接结构被破坏，是否意味着空间域 message passing（局部、拓扑无关）天然更适合 AMR 场景？
3. 结构保持图学习：在消息函数中嵌入守恒律/对称性（如 SE(3) 等变消息、通量平衡约束、Casimir/能量守恒损失）相比纯监督 MPNN，在长期 roll-out 稳定性与泛化上的增益能否用核/谱语言（等变 NTK、群平均核）定量刻画？库内 Geometric DL 与 NTK 文献能否直接支撑这类分析？

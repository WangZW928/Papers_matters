...
> 注：这是 1907 年的启发式弱场表述，仅在一阶近似下成立。采用通常的时空号差并对静止观察者，现代 GR 在静态时空中的精确关系为 $d\tau=\sqrt{-g_{00}}\,dt$；只有在弱场参数化 $g_{00}\simeq-(1+2\Phi/c^2)$ 下，才有 $d\tau\simeq(1+\Phi/c^2)dt$。除非把 $\Phi$ 特别定义为由 $g_{00}$ 给出的相对论势，否则不能把 $\sqrt{1+2\Phi/c^2}$ 称为一般情形的精确公式。
